...
// {
//     [Header("Target")]
//     public Transform playerTransform; // 따라다닐 플레이어

//     [Header("Settings")]
//     public float radius = 3f;
//     public float rotationSpeed = 100f;
//     public float followSmoothTime = 0.05f; // 따라가는 부드러움 정도
...
//         // 1. 플레이어 위치 따라가기 (SmoothDamp를 써서 부드럽게 추적)
...
//             // 로컬 좌표계를 기준으로 위치 계산 (자신이 플레이어를 따라가고 있으므로)
//             Vector3 offset = new Vector3(Mathf.Cos(radians), 0, Mathf.Sin(radians)) * radius;
//             _activeOrbitals[i].transform.localPosition = offset;
            
//             // 옵션: 크리스탈이 항상 중앙(플레이어 쪽)을 바라보게 함
...
//         {
//             GameObject target = _activeOrbitals[0];
//             _activeOrbitals.RemoveAt(0);
//             Destroy(target);
